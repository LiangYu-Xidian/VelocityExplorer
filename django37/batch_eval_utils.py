...
def batch_cross_boundary_correctness(
    ...
    for u, v in cluster_edges:
        ...
        for i in range(len(index)):
            ...
            different_batch_boundary_nodes.append(
                keep_different_batch(adata, boundary_nodes[i], batch, k_batch=k_batch))
...
def batch_inner_cluster_coh(adata, k_cluster, k_velocity, k_batch="batch", return_raw=False):
    ...
    for cat in clusters:
        ...
        for i in range(len(index)):
            batch = adata.obs[k_batch][index[i]]
            different_batch_same_cat_nodes.append(
                keep_different_batch(adata, same_cat_nodes[i], batch, k_batch=k_batch))
